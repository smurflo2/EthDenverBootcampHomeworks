...
describe("VolcanoCoin", function() {
    let VolcanoCoinDeployer, volcanoCoin, owner, addr1, addr2;
    ...
    describe("Security", () => {
        it("Only the owner should be able to increase supply", async () => {
            await expect(() => volcanoCoin.connect(addr1).increaseSupply().to.be.revertedWith("Ownable: caller is not the owner"))
        })
    })
})
